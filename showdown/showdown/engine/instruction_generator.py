...
def get_state_from_volatile_status(mutator, volatile_status, attacker, affected_side, first_move, instruction):
    ...
    if can_be_volatile_statused(side, volatile_status, first_move) and volatile_status not in side.active.volatile_status:
        ...
        if volatile_status == constants.SUBSTITUTE:
            ...
            instruction.add_instruction(
                    (
                        'subtract_substitute_hp',
                        affected_side,
                        np.floor(-1 * side.active.maxhp * 0.25)
                    )
            )
    else:
        mutator.reverse(instruction.instructions)
...
def get_instructions_from_switch(mutator, attacker, switch_pokemon_name, instructions):
    ...
    if attacking_side.active.substitute_hp != 0:
        sub_reset_instruction = (
            'subtract_substitute_hp',
            attacker,
            attacking_side.active.substitute_hp
        )
        mutator.apply_one(sub_reset_instruction)
        instruction_additions.append(sub_reset_instruction)

    if attacking_side.active.part_trapped_counter != 0:
        partially_trapped_reset_instruction = (
            'subtract_part_trapped',
            attacker,
            attacking_side.active.part_trapped_counter
        )
        mutator.apply_one(partially_trapped_reset_instruction)
        instruction_additions.append(partially_trapped_reset_instruction)
    ...
    if switch_pkmn.item != 'heavydutyboots':
        ...
        if attacking_side.side_conditions[constants.STEALTH_ROCK] == 1 and switch_pkmn.ability != 'magicguard':
        ...
        if attacking_side.side_conditions[constants.SPIKES] > 0 and switch_pkmn.is_grounded() and switch_pkmn.ability != 'magicguard':
        ...
        if attacking_side.side_conditions[constants.STICKY_WEB] == 1 and switch_pkmn.is_grounded() and switch_pkmn.ability not in constants.IMMUNE_TO_STAT_LOWERING_ABILITIES  and switch_pkmn.ability != 'magicguard':
...
def get_instructions_decrement_pp(mutator, attacking_move, attacking_pokemon, defending_pokemon, attacking_side, instruction):
    if instruction.frozen or attacking_move['id'] == 'splash':
        return [instruction]
    #print(attacking_move['id'], attacking_pokemon, defending_pokemon)
    mutator.apply(instruction.instructions)
    instruction_additions = []
    if defending_pokemon.ability != 'pressure':
        #x['current_pp'] -= 1
        instruction_additions += [('decrement_pp', attacking_side, attacking_move['id'], 1)]
    else:
        #x['current_pp'] -= 2
        instruction_additions += [('decrement_pp', attacking_side, attacking_move['id'], 2)]
    for x in attacking_pokemon.moves:
        if x['id'] == attacking_move['id'] and x['current_pp'] <= 0:
            #x['disabled'] = True
            instruction_additions += [('disable_move',attacking_side, attacking_move['id'])]
...
def get_states_from_damage(mutator, defender, damage, accuracy, attacking_move, instruction):
    ...
    if percent_hit > 0:
        if constants.SUBSTITUTE in damage_side.active.volatile_status and constants.SOUND not in move_flags and attacker_side.active.ability != 'infiltrator':
            hit_sub = True
            if damage >= damage_side.active.substitute_hp:
                actual_damage = damage_side.active.substitute_hp
                instruction_additions.append(
                    (
                        constants.MUTATOR_REMOVE_VOLATILE_STATUS,
                        defender,
                ...
                instruction_additions.append(
                    (
                        'subtract_substitute_hp',
                        defender,
                        damage_side.active.substitute_hp
                    )
                )
            else:
...
def get_instructions_from_side_conditions(mutator, attacker_string, side_string, condition, instruction):
    ...
    if condition == constants.WISH:
        ...
        if side.side_conditions[condition] < max_layers:
            ...
            if condition == constants.LIGHT_SCREEN:
                if side.active.item == 'lightclay':
                    instruction_additions.append(
                        (
                            'increment_ls_counter',
                            side_string,
                            8
                        ))
                else:
            ...
            elif condition == constants.REFLECT:
                if side.active.item == 'lightclay':
                    instruction_additions.append(
                        (
                            'increment_reflect_counter',
                            side_string,
                            8
                        ))
                else:
...
def get_end_of_turn_instructions(mutator, instruction, bot_move, opponent_move, bot_moves_first):
    ...
    for attacker in sides:
        ...
        if attacker == 'self':
            ...
            opp = 'self'
        opp_side = get_side_from_state(mutator.state, opp)
        pkmn = side.active
        ...
        if constants.PARTIALLY_TRAPPED in pkmn.volatile_status:
            ...
            if pkmn.part_trapped_counter > 0 and pkmn.hp > 0 and opp_side.active.hp > 0:
                partially_trapped_dec_instruction = (
                    'subtract_part_trapped',
                    attacker,
                    1
                )
                mutator.apply_one(partially_trapped_dec_instruction)
                instruction.add_instruction(partially_trapped_dec_instruction)
            else:
                partially_trapped_reset_instruction = (
                    'subtract_part_trapped',
                    attacker,
                    pkmn.part_trapped_counter
                )
                mutator.apply_one(partially_trapped_reset_instruction)
                instruction.add_instruction(partially_trapped_reset_instruction)
    ...
    for attacker in sides:
        ...
        if side.side_conditions != None:
            for condition in side.side_conditions:
                if condition == constants.LIGHT_SCREEN and side.side_conditions[condition] > 0:
                    if side.ls_count > 0:
                        screen_dec_instruction = (
                            'increment_ls_counter',attacker,
                            -1
                        )
                        mutator.apply_one(screen_dec_instruction)
                        instruction.add_instruction(screen_dec_instruction)
                    else:
                        screen_dec_instruction = (
                            'increment_ls_counter',attacker,
                            -1 * side.ls_count
                        ...
                        mutator.apply_one(screen_dec_instruction)
                        instruction.add_instruction(screen_dec_instruction)
                        clear_screen_instruction = (constants.MUTATOR_SIDE_START, attacker,
                                            constants.LIGHT_SCREEN, -1 * side.side_conditions[constants.LIGHT_SCREEN])
                        mutator.apply_one(clear_screen_instruction)
                        instruction.add_instruction(clear_screen_instruction)

                elif condition == constants.REFLECT and side.side_conditions[condition] > 0:
                    if side.ref_count > 0:
                        screen_dec_instruction = (
                            'increment_reflect_counter',attacker,
                            -1
                        )
                        mutator.apply_one(screen_dec_instruction)
                        instruction.add_instruction(screen_dec_instruction)
                        ...
                        instruction.add_instruction(screen_dec_instruction)
                        clear_screen_instruction = (constants.MUTATOR_SIDE_START, attacker,
                                            constants.REFLECT, -1 * side.side_conditions[constants.REFLECT])
                        mutator.apply_one(clear_screen_instruction)
                        instruction.add_instruction(clear_screen_instruction)

                elif condition == constants.AURORA_VEIL and side.side_conditions[condition] > 0:
                    if side.av_count > 0:
                        screen_dec_instruction = (
                            'increment_av_counter',attacker,
                            -1
                        )
                        mutator.apply_one(screen_dec_instruction)
                        instruction.add_instruction(screen_dec_instruction)
                        ...
                        instruction.add_instruction(screen_dec_instruction)
                        clear_screen_instruction = (constants.MUTATOR_SIDE_START, attacker,
                                            constants.AURORA_VEIL, -1 * side.side_conditions[constants.AURORA_VEIL])
                        mutator.apply_one(clear_screen_instruction)
                        instruction.add_instruction(clear_screen_instruction)

    # Weather COUNT
    if mutator.state.weather != None:
        if mutator.state.weather_count > 0:
            weather_dec_instruction = (
                'increment_weather',
                -1
            )
            mutator.apply_one(weather_dec_instruction)
            instruction.add_instruction(weather_dec_instruction)
        else:
            weather_dec_instruction = (
                'increment_weather',
                -1 * mutator.state.weather_count
            )
            mutator.apply_one(weather_dec_instruction)
            instruction.add_instruction(weather_dec_instruction)
            clear_weather_instruction = (
                constants.MUTATOR_WEATHER_START,
                None,
                mutator.state.weather
            )
            mutator.apply_one(clear_weather_instruction)
            instruction.add_instruction(clear_weather_instruction)

    # TERRAIN COUNT
    if mutator.state.field != None:
        if mutator.state.terrain_count > 0:
            terrain_dec_instruction = (
                'increment_terrain',
                -1
            )
            mutator.apply_one(terrain_dec_instruction)
            instruction.add_instruction(terrain_dec_instruction)
        else:
            terrain_dec_instruction = (
                'increment_terrain',
                -1 * mutator.state.terrain_count
            )
            mutator.apply_one(terrain_dec_instruction)
            instruction.add_instruction(terrain_dec_instruction)
            clear_terrain_instruction = (
                constants.MUTATOR_FIELD_START,
                None,
                mutator.state.field
            )
            mutator.apply_one(clear_terrain_instruction)
            instruction.add_instruction(clear_terrain_instruction)
...
def remove_volatile_status_and_boosts_instructions(side, side_string):
    ...
    if side.active.evasion_boost:
        instruction_additions.append(
            (
                constants.MUTATOR_UNBOOST,
                side_string,
                constants.EVASION,
                side.active.evasion_boost
            ))
