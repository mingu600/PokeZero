...
def user_moves_first(state, user_move, opponent_move):
    ...
    if constants.SWITCH_STRING in user_move and constants.SWITCH_STRING in opponent_move:
        if user_effective_speed == opponent_effective_speed:
...
def get_state_instructions_from_move(mutator, attacking_move, defending_move, attacker, defender, first_move, instructions):
    ...
    all_instructions = instruction_generator.get_instructions_decrement_pp(mutator, attacking_move, attacking_pokemon, defending_pokemon, attacker, instructions)
    ...
    if damage_amounts is not None:
        temp_instructions = []
        for instruction_set in all_instructions:
            amount_of_damage_rolls = len(damage_amounts[0])
            amount_of_crit_damage_rolls = len(damage_amounts[1])
            for dmg in damage_amounts[0]:
                these_instructions = copy(instruction_set)
                these_instructions.update_percentage(23/24 * 1 / amount_of_damage_rolls)
                temp_instructions += instruction_generator.get_states_from_damage(mutator, defender, np.ceil(dmg), move_accuracy, attacking_move, these_instructions)
            for dmg in damage_amounts[1]:
                these_instructions = copy(instruction_set)
                these_instructions.update_percentage(1/24 * 1 / amount_of_crit_damage_rolls)
                temp_instructions += instruction_generator.get_states_from_damage(mutator, defender, np.ceil(dmg), move_accuracy, attacking_move, these_instructions)
        all_instructions = temp_instructions
